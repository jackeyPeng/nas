...
| 版本 | 日期 | 主要变化 |
|------|------|---------|
| v1.4.0-beta.10 | 2026-09-18 | 首登强制改密弹窗全局化（不再需进用户管理页才出现）+ install.sh 步骤4 耗时提示细化与 setup.sh 步骤编号统一 + 仓库地址补全 GitHub 镜像 |
...
| v1.4.0-beta.4 | 2026-09-04 | SMB 按用户读写权限 + 协议诚实标注 + 安装完成显示真实凭证 |
| v1.4.0-beta.3 | 2026-09-04 | 发布产物安全清洗 + 部署脚本参数化（去除硬编码 IP/密码） |
| v1.4.0-beta.2 | 2026-09-03 | 权限模型跨服务联动修复 + 配置生成引擎健壮化 |
| v1.4.0-beta.1 | 2026-09-02 | 全站 i18n（中英 970 键）+ 移动端 PWA + HTTPS 证书 + 系统诊断 |
| v1.3.0 | 2026-08-03 | 存储四层模型 + Design System v2.0 + rclone 增强 + 防火墙重写 + Architecture v1.0 |
| v1.2.0 | 2026-07-13 | 模块化重构 + 配置管理 + 备份恢复 + rclone serve s3 + 硬件定义 |
...
## [2026-09-18] - 首登改密弹窗全局化 + 安装体验收尾

### 版本 v1.4.0-beta.10

### 修复

- 首登强制改密弹窗全局化：弹窗从用户管理页容器移到 `x-show=token` 全局层（z-index 10000）——此前登录落在 dashboard 时 x-show 祖先不满足导致弹窗不渲染，必须点进用户管理页才出现；刷新恢复 token 后 init() 也重查默认密码，弹窗不再因会话恢复而丢失
- install.sh 步骤4提示细化：测速/update/下载分包各自显示 + 实时耗时计时器（每秒刷新）；setup.sh 步骤编号统一为 [N/10]

### 文档

- 所有仓库地址展示处补充 GitHub 镜像 github.com/jackeyPeng/nas（与 Gitee 主仓并列）
...
## [2026-09-11] - 商业化 P0 全落地 + P1 健康中心/调度器/SBOM

### 版本 v1.4.0-beta.7

### 商业化 P0（7/7 全部完成）

- Import Pool 换机恢复：系统盘重装后导入既有 LVM 池，folders.db 自动重建
- OTA 签名校验：ed25519 签名 + SHA256，面板一键升级 + 失败自动回滚
- 2FA/TOTP 两步验证（默认关闭，纯标准库实现）
- 危险操作服务端 type-to-confirm（删池/格式化/重置需输入确认词）
- 凭证保险箱：AES-256-GCM 加密存储（web/modules/vault）
- License 页 + 源码链接（AGPL §13 网络源码义务）
- 配置 vs 数据备份 UI 区分

### P1 产品化

- 健康中心 Health Center：`/api/health` 聚合系统/存储/网络/备份/安全/云端同步 6 项检查
- rclone 定时同步调度器：cron 表达式，每 30s 扫描，支持 */,-/步进 + 分钟去重 + 防并发
- SBOM 自动生成：`scripts/gen-sbom.py` 产出 manifest.json + SBOM.spdx.json（SPDX 2.3），release.sh 集成
...
- 双机（.48/.57）全新安装 + 完整用户旅程验收 13 项全过：建池选盘 / 46 注册表 / SMB 隔离 / 备份 / 保险箱 / 健康中心 / 危险操作 / Import Pool / 调度器
...
- deploy-nas-panel.sh 目标改为命令行参数传入（`[user@]host`），删除写死的 SERVERS 数组与 IP→用户映射
- install-services.sh 账号密码改从环境变量或 /opt/nas/.env 读取，去掉硬编码密码与用户名
- 删除 deploy-115.sh（一次性内网部署脚本，含硬编码 IP + 明文密码）
- 用 git-filter-repo 重写全部历史，清除脚本/文档/CHANGELOG/TODO 中的内网 IP 与明文密码
- 文档中的示例 IP 统一替换为通用示例 192.168.1.100 / 192.168.1.0/24

### 2. 验证

- 从 gitee.com 全新 clone 扫描，无内网 IP / 明文密码残留，253 commits，tag 齐全
...
### 1. i18n 多语言全站完成（393 → 970 keys）

- 语言包从 393 扩到 970 keys，中英文完全一致（`scripts/check-i18n.py` 校验通过）
- 全站三路文案全部走 i18n：
  - 静态文本：index.html 约 349 个文本节点 → `$t()`
  - 运行时文案：app.js 约 90 处 confirm/toast/预设标签 → `this.t()`
  - 后端数据型文案：前端按稳定 ID/name/key 映射（服务名/描述、存储池/卷、组件详情、sysctl 描述、诊断项、RAID 方案、告警渠道、防火墙预设）
- 28 处中文 placeholder 改为 `:placeholder="$t(...)"` 绑定
- 成功提示不再被后端中文 `message` 覆盖（`data.message || this.t()` 翻转为纯 `this.t()`）
- 修复两个关键 bug：
  - `$t` 未定义导致全站文字空白（`window.$t = t` + 语言包同步加载）
  - 切换语言页面不刷新（`$t` 注册为 Alpine magic，读取响应式 `store('i18n').lang`）

### 2. 移动端 PWA 支持

- `manifest.json` + theme-color，可「添加到主屏幕」
- `sw.js` 离线缓存静态资源
- 192/512 两尺寸图标

### 3. 前端集成四模块 UI

- 系统设置页新增「HTTPS 证书」配置区块（状态卡 + 自签名生成 + 自定义上传 + 应用到服务/移除）
- 系统诊断页（SMART 短检/长检 + RAID scrub）
- 汉堡菜单移动端抽屉导航 + 响应式布局（@media 768px）
...
## [2026-08-28] - 多语言框架 + HTTPS 证书 + 系统诊断
...
- 完整 i18n 框架：`i18n.js` 自动检测浏览器语言，动态加载 JSON 语言包
- 语言包：zh-CN.json (393 keys) + en-US.json (393 keys) 完整翻译
- 侧边栏语言切换下拉框，即时生效，存 localStorage
- 核心模块文本已替换为 `$t()` 调用（~260 处）
- 同步检查脚本 `scripts/check-i18n.py`（`--fix` 自动补充缺失 key）
- 添加新语言：复制 JSON 翻译 + 2 行注册即可

### 2. HTTPS 证书配置

- 系统设置 → HTTPS 证书：一键生成自签名证书 + 上传自定义证书
- 自签名证书：openssl 生成，10 年有效，默认域名=主机名.local
- 自定义证书：PEM 格式上传，带证书/私钥匹配校验
- 自动应用到 4 个 Web 服务（面板/FileBrowser/WebDAV/S3）
- 自动更新 systemd unit 文件并重启服务
...
- 导航「维护 → 🔬 系统诊断」
- 三个诊断项：SMART 短检（2分钟）/ SMART 长检（4小时）/ RAID 数据清理（2小时）
- 手动触发 + 自动调度（每天/每周/每月）
- 保护机制：时间窗口限制、温度上限 55°C、IO 使用率保护 70%、RAID 限速
- 诊断历史记录（SQLite，保留 500 条）
