...
**简介：** 存储管理是 NAS 面板的核心功能，采用三层抽象模型（物理磁盘 → 存储空间 → 共享文件夹），参考群晖/TrueNAS 设计，屏蔽底层路径，用户只看到"存储空间1"等友好名称。
...
- **头部**：存储空间名（存储空间1）+ RAID 级别标签 + 文件系统 + 容量进度条
- **磁盘详情**：设备路径 + 容量 + 接口 + 型号 + 温度 + SMART 状态
- **共享文件夹**：名称 + 大小 + 权限标签（读写/只读/未共享）+ 回收站图标
- **操作按钮**：扩容（仅 LVM 模式）/ 重置

### 存储配置向导

检测到空闲磁盘时自动显示，根据磁盘数量推荐方案：
