...
5. **发布产品不写内网 IP / 明文密码**（2026-09-04 确立）：安装/部署脚本账号密码从环境变量 / `/opt/nas/.env` 读取，禁止硬编码真实密码；一次性内网脚本（deploy-115.sh）不进仓库；文档示例 IP 用 192.168.1.100 这类通用示例。发版前 `grep -rnE "10\.216\.|10\.187\.|192\.168\.213\.|nas123456" scripts/ docs/ configs/` 必须为空。详见 `docs/release-system.md`。
